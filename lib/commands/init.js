...
module.exports = function (path = '', local, theme, plugins) {
  ...
  if (plugins) {
    plugins.forEach(plugin => {
      replace(target(filename), '_plugins_', '_plugin_\n  _plugin_')
      const scriptTemplate = `<script src="//cdn.jsdelivr.net/npm/docsify/lib/${plugin}.min.js"></script>`
      replace(target(filename), '_plugin_', scriptTemplate)
    })
  } else {
