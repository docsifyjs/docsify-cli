...
function generateSidebarOfDirectory(baseDirectory, currentDirectory, indentCount) {
  ...
  for (const subDirectory of subDirectories) {
    const contentOfDirectory = generateSidebarContentOfDirectory(currentDirectory, indentCount)
    console.log(contentOfDirectory)
    lines.push(contentOfDirectory)
  ...
  for (const mdFilePath of mdFilePaths) {
    const mdFilePathLink = generateSidebarLinkOfFile(baseDirectory, mdFilePath, indentCount)
...
function generateLinkOfFile(baseDirectory, filePath) {
  ...
  return ''.concat('[', name, ']', '(', relativePath, ')')
}
